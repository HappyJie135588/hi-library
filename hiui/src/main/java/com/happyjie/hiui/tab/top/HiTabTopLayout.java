...
public class HiTabTopLayout extends HorizontalScrollView implements IHiTabLayout<HiTabTop, HiTabTopInfo<?>> {
     ...
     * @param nextInfo
     */
    private void autoCenter(HiTabTopInfo nextInfo) {
        HiTabTop tabTop = findTab(nextInfo);
        if (tabTop == null) return;
        int screenWidth = HiDisplayUtil.getDisplayWidthInPx(getContext());
        int rb_px = (int) tabTop.getX() + tabTop.getWidth() / 2;
        scrollTo(rb_px - screenWidth / 2, 0);
    }
